...
const emitter = new Emitter({
    ...
    calls: {
        ...
        sign: mathFn("sign"),
        // @mod is Zig's compiler builtin for floor-mod specifically
        // (Zig distinguishes it from @rem, the truncated/sign-of-
        // dividend version, precisely for this case) -- no sign
        // correction needed. std.math.clamp matches clampIndex directly.
        wrapIndex: ([i, m]) => `@mod(${i}, ${m})`,
        clampIndex: ([i, lo, hi]) => `std.math.clamp(${i}, ${lo}, ${hi})`,
    },
    // Zig slice indexing requires a usize index. @intFromFloat's return
    // type is inferred from context -- @as(usize, ...) supplies that
    // context inline rather than needing a separate typed local.
    // Verified directly against a real compile (Zig 0.14.1): a slice
    // indexed via arr[@as(usize, @intFromFloat(i))] read back the
    // correct element.
    emitIndex: function (targetNode, atNode) {
        return `${this.emitExpr(targetNode)}[@as(usize, @intFromFloat(${this.emitExpr(atNode)}))]`;
    },
    ...
    formatFunction: (fn, body, letBindings = [], guardLines = [], opts = {}) => {
        ...
        const params = fn.params.map((p) => `${p}: ${fn.paramTypes?.[p] === "number[]" ? "[]const f64" : "f64"}`).join(", ");
        ...
        // directly (a real zig build-exe) that this has the SAME
        // duplicate-definition bug reported for QB64's own
        // SAFE_MATH_HELPERS: concatenating N functions into one .zig
        // file duplicates "const std = @import(...)" N times --
        // "duplicate struct member name 'std'". Unlike QB64/COBOL's own
        // helper blocks, this one's safe to gate unconditionally with no
        // per-function usage check needed: confirmed directly that an
        // UNUSED top-level/file-scope const (unlike a local one -- see
        // the zero-param fix elsewhere in this file's own history) is
        // NOT an error in Zig, so omitting it for every call after the
        // first never risks leaving a function that actually needs it
        // without it. See docs/multi-function-files.md.
        const stdImport = opts.includeHelpers === false ? "" : `const std = @import("std");\n\n`;
